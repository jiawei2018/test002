...
public class gasStation_134 {
    public static void main(String[] args) {
        ...
        //int[] g = {3,3,4};
       // int[] c = {3,4,4};
        gasStation_134 t = new  gasStation_134();
        t.canCompleteCircuit(g,c);
    ...
    private int isok(int[] gas, int[] cost, int idx){
        int tmpidx = idx;
        int curgas = 0;
        int count = 0;

        while(curgas >= 0){
            if(count == gas.length) return idx;
            curgas += gas[tmpidx] - cost[tmpidx];
            if(tmpidx < gas.length - 1){
                tmpidx++;
            }else
                tmpidx = 0;
            count++;
